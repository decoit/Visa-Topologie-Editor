...
/**
 * This HttpHandler implementation queues a request to change the network of an
 * interface. The change is not executed, it is queued in a
 ...
 * @see DefaultHandler
 */
public class QueuedModifyInterfaceIPHandler extends DefaultHandler {
	private static Logger log = Logger.getLogger(QueuedModifyInterfaceIPHandler.class.getName());
	...
	public void handle(HttpExchange he) throws IOException {
		...
		if(this.checkQueryParameters(queryParameters)) {
			...
			try {
				...
				mq.addModification(ModificationTarget.INTERFACE, queryParameters.get("ifID").get(), ModificationTargetAttribute.INTERFACE_NETWORK, queryParameters.get("net").get());
	...
	private boolean checkQueryParameters(QueryString pQueryString) {
		...
		if(rv && (!pQueryString.containsKey("net") || pQueryString.get("net").get().isEmpty())) {
			rv = false;
		}
