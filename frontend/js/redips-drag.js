...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		save_content,				// scan tables, prepare query string and sent to the multiple-parameters.php
				...
				getTblCell,                             // Addition by Thomas Rix, needed to get the DOM object of the current table cell
				checkDIVBounds,                         // Addition by Thomas Rix,
	...
	save_content = function (tbl) {
		...
		getTblCell = function() {
			if(table < tables.length) {
				return tables[table].rows[row].cells[cell];
			}
			else {
				return null;
			}
	...
	return {
		...
		save_content		: save_content,
				...
				// Modification by Thomas Rix (DECOIT GmbH), calculate_cells needed in myhandlers
				// to allow the whole drag-DIV to be clicked after dynamically changing
				// row- and colspan. Without recalculation, only the first "row" ob the DIV
				// can be clicked and dragged
				calculate_cells         : calculate_cells,

				// Modification by Thomas Rix (DECOIT GmbH), set_trc is needed to track
				// mouse movement while drawing a cable
				// getTblCell returns the DOM object of the table cell the mouse is
				// currently hovering
				set_trc                 : set_trc,
				getTblCell              : getTblCell,
				checkDIVBounds          : checkDIVBounds,
